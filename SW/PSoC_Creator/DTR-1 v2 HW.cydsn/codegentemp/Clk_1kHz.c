...
void Clk_1kHz_Start(void) 
{
    ...
    Clk_1kHz_CLKEN |= Clk_1kHz_CLKEN_MASK;
	Clk_1kHz_CLKSTBY |= Clk_1kHz_CLKSTBY_MASK;
}
...
void Clk_1kHz_Stop(void) 
{
    ...
    Clk_1kHz_CLKEN &= (uint8)(~Clk_1kHz_CLKEN_MASK);
	Clk_1kHz_CLKSTBY &= (uint8)(~Clk_1kHz_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clk_1kHz_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clk_1kHz_StopBlock(void) 
{
    if ((Clk_1kHz_CLKEN & Clk_1kHz_CLKEN_MASK) != 0u)
    {
...
#if defined(Clk_1kHz__CFG3)
        CLK_DIST_AMASK = Clk_1kHz_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clk_1kHz_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clk_1kHz__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clk_1kHz_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clk_1kHz_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clk_1kHz_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clk_1kHz_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        Clk_1kHz_CLKSTBY &= (uint8)(~Clk_1kHz_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: Clk_1kHz_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clk_1kHz_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = Clk_1kHz_GetSourceRegister();
    uint16 oldDivider = Clk_1kHz_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clk_1kHz_CLKEN & Clk_1kHz_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clk_1kHz_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
{
    return CY_GET_REG16(Clk_1kHz_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clk_1kHz_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clk_1kHz_SetModeRegister(uint8 modeBitMask) 
{
    Clk_1kHz_MOD_SRC |= modeBitMask & (uint8)Clk_1kHz_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clk_1kHz_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clk_1kHz_ClearModeRegister(uint8 modeBitMask) 
{
    Clk_1kHz_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clk_1kHz_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clk_1kHz_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clk_1kHz_GetModeRegister(void) 
{
    return Clk_1kHz_MOD_SRC & (uint8)(Clk_1kHz_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clk_1kHz_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clk_1kHz_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = Clk_1kHz_GetDividerRegister();
    uint8 oldSrc = Clk_1kHz_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clk_1kHz_MOD_SRC |= CYCLK_SSS;
        Clk_1kHz_MOD_SRC =
            (Clk_1kHz_MOD_SRC & (uint8)(~Clk_1kHz_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clk_1kHz_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clk_1kHz_GetSourceRegister(void) 
{
    return Clk_1kHz_MOD_SRC & Clk_1kHz_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clk_1kHz_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clk_1kHz_SetPhaseRegister(uint8 clkPhase) 
{
    Clk_1kHz_PHASE = clkPhase & Clk_1kHz_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clk_1kHz_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clk_1kHz_GetPhaseRegister(void) 
{
    return Clk_1kHz_PHASE & Clk_1kHz_PHASE_MASK;
}
